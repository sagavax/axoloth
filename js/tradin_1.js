...
document.getElementById("add_asset").addEventListener("click", function() {
    ...
    if (asset_name === "") {
        ...
        var platf_id = localStorage.getItem("platf_asset_id");
        xhttp.open("POST", "asset_add.php", true);
        xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhttp.send("asset_name=" + encodeURIComponent(asset_name) + "&asset_symbol=" + encodeURIComponent(asset_symbol) + "&asset_url=" + encodeURIComponent(asset_url) + "asset_type=" + encodeURIComponent(asset_type) + "&asset_description=" + encodeURIComponent(asset_description) + "&platform_id=" + platform_id);
    }
})
...
function reload_platform_notes(platform_id) {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            document.querySelector(".trading_feed").innerHTML =
                this.responseText;
        }
    };
...
document.getElementById("search_platform").addEventListener("keyup", function() {
    var search_asset = document.getElementById("search_asset").value;
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            document.querySelector(".trading_platforms").innerHTML =
