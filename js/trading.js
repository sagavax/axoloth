...
document.getElementById("modal_add_note").addEventListener("click", function() {
    var el = document.querySelector(".trading_note");
    el.style.display = (el.style.display == 'none' || el.style.display == '') ? 'flex' : 'none';
})
...
//edit note
var edit_note = document.querySelectorAll(".edit_note");
console.log(edit_note);
for (var i = 0; i < edit_note.length; i++) {
    edit_note[i].addEventListener("click", function() {
        alert("edit note");
    })
}

//delete note 
var remove_note = document.querySelectorAll(".remove_note");
console.log(remove_note);
for (var i = 0; i < remove_note.length; i++) {
    remove_note[i].addEventListener("click", function() {
        alert("remove note");
    })
}
...
document.getElementById("add_note").addEventListener("click", function() {
    ...
    var latest_id = localStorage.getItem('latest_id');
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                parent.insertAdjacentHTML("afterbegin",
                    `<div class='note' note-id=` + latest_id + `>
...
document.getElementById("add_trading_note").addEventListener("click", function() {
    //get note title
    var note_title = document.getElementById("trading_note_title").value;
    //get note text
    var note_text = document.getElementById("trading_note_text").value;
    //alert(note_title + " " + note_text);
    if (note_text === "") {
        alert('Cannot be empty!!!');
    } else {
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                document.getElementById("trading_note_title").value = "";
                document.getElementById("trading_note_text").value = "";
                document.querySelector(".trading_note").style.display = "none";
            }
        }
    }

    xhttp.open("POST", "trading_note_add.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    data = "note_title=" + note_title + "&note_text=" + encodeURIComponent(note_text);
    xhttp.send(data);
});
...
document.getElementById("hide_trading_note").addEventListener("click", function() {
    var el = document.querySelector(".trading_note");
    el.style.display = "none";
});
