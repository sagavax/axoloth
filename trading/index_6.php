...
while ($row = mysqli_fetch_array($result)) {
	...
	echo "<li><div class='platform' platforn-id=$id platform-url=$platform_url><div class='platform_name'>$platform</div><div class='platf_actions'><button class='btn-small add_platf_note' type='button' title='add note'><i class='fa fa-plus'></i></button><button class='btn-small add_platf_asset' type='button' title='add asset'><i class='fa fa-plus' class=''></i></button><button type='button' class='btn-small action_menu'><i class='fas fa-ellipsis-v'></i></button></div><div class='pl_menu'></div></li>";

	$notes = PlatformHasNotes($id);
	//echo "<script>alert($notes)</script>";
	if (PlatformHasNotes($id) > 0) {
		//if pltform has any news/notes/assets
...
} // get all trading platform and their notes
?>
                        </ul>
                    </div>
...
while ($row = mysqli_fetch_array($result)) {
	...
	echo "<div class='asset asset-id=$asset_id'>";
	echo "<div class='asset_name'>$asset_name</div>";
	echo "<div class='asset_symbol'>$asset_symbol</div>";
	echo "<div class='asset_action'><button type='button' class='btn-small remove_asset'><i class='fa fa-edit'></i></button><button type='button' class='btn-small remove_asset'><i class='fa fa-times'></i></button></div>";
	echo "</div>";
}
?>
                            </div>
...
?>
    ...
    <script src='../js/trading.js?<?php echo time(); ?>'></scrip>
</body>
</html>
