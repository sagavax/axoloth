...
?>"></td>
                      ...
                      <tr>
                        <td>Event date (end):</td><td><input type="date"  onchange="date_validation();" id="end_date" data-toggle="datepicker" name="end_event_date" value="<?php
if (isset($_GET['day'])) {
	echo date("Y-m-d", strtotime($_GET['day']));
} else {
	echo date("Y-m-d");
...
?>"></td>
                      ...
                      <?php $colors = array("#ede7f6", "#9575cd", "#b3e5fc", "#4fc3f7", "#b2dfdb", "#4db6ac", "#c5e1a5", "#aed581", "#ffcdd2", "#ef9a9a", "#fcdc29", "#7ba8ef", "#b3cff3", "#fb819b", "#8bc34a"); //zoznam farieb
$random_color = array_rand($colors, 1);
echo "<input type='hidden' name='event_color' value=$colors[$random_color]>";
?>
                      <tr>
