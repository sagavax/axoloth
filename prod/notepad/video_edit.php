...
if (isset($_POST['edit_video'])) {
   ...
   $sql = "UPDATE tblcustomer_notes set cust_id=$customer,title='$note_title', note_source='$note_source',note_type='video' where id=$note_id";
   global $con;
   $result = mysqli_query($con, $sql)  or die("MySQL ERROR: " . mysqli_error($con));

   //updatnem wall
   $link = mysqli_connect(null, "brick_wall", "h3jSXv3gLf", "brick_wall", null, "/tmp/mariadb55.sock");
   //$link=mysqli_connect('localhost','root',null,'brick_wall',null,null);

   //zapisem do logu       
   $curr_date = date('Y-m-d H:i:s');
   $curr_action = "modify_video";
   $curr_app = "notepad";
   //$new_note_id=GetLatestNote();
   $text_logu = "The video id $note_id has been modified";
   ...
   $result = mysqli_query($con, $sql)  or die("MySQL ERROR: " . mysqli_error($link));
   echo "<script>alert('The video id $note_id has been modified');
      ...
      <div class="add_new_note_wrap">
         ...
         ?>
            <form action="" method="post" id="edit_note_form">
               ...
               <div class="edit_note_title"><input type="text" name="note_title" value="<?php echo $title ?>"></div>
               ...
               <div class="edit_note_action">
                  <button name="edit_video" type="submit" class="flat-btn">Save changes</button></td>
               </div>
