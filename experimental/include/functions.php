...
<?php
   ...
   /*  
     function get_list_of_week_events(){
      $dt = new DateTime();
      $dates = [];
      for ($d = 1; $d <= 7; $d++) {
      ...
      // echo $value."<br>";
       ...
       while ($row = mysqli_fetch_array($result)) {
           ...
           $start_even_date = $row['start_event_date'];
           $end_event_date = $row['end_event_date'];
            //var_dump($row);
           echo "<li>";
           echo "<div class='event' id='$event_id'>
               <div class='event_title'>$event_title</div><div class='event_start'>$start_even_date</div><div class='event_end'>$end_event_date</div><div class='event_actions'><ul><li><a href='event.php?event_id=$event_id' class='flat-btn'>View</a></li><li><a href='event_edit.php?event_id=$event_id' class='flat-btn'>Edit</a></li><li><a href='event_remove.php?event_id=$event_id' class='flat-btn'>Remove</a></li></ul></div>
               </div>";
           echo "</li>";    
       }   //end while */
  /*     echo "</ul>";
...
*/
     ...
     function Is_note_synced($note_id){ //funkcia detekuje ci je tato poznamka/ text v minecraft is 
      global $con;
     ...
     function Task2Sync2MinecraftIS($task_id){
        ...
        if($category_id==5){//yes it is minecraft task
    ...
    $result=mysqli_query($con, $sql) or die("MySQL ERROR: ".mysqli_error());
          ...
          if ($num>0) // mame nejaky event planovany na tento den
               ...
               {  //something we have here
                  ...
                  $calendar.= "<td class='calendar-event-day' onclick='show_alert($date)' data-date=$date>";
                } elseif ($date == $today ) { //just today
                  
                  $calendar.= "<td class='calendar-today' onclick='show_alert($today)' data-date=$date>";
                } else { //no event for this day
     ...
     $calendar.= "<div class='day-number' id='$event_date'><a href='index.php?view_day=$year-$month-$list_day'>$list_day</a></div>";
      /** QUERY THE DATABASE FOR AN ENTRY FOR THIS DAY !!  IF MATCHES FOUND, PRINT THEM !! **/
     // $calendar.= str_repeat('<p> </p>',2);
    ...
    //$calendar.= "<td class='calendar-day' data-date=".$date.">"; 
          {
            ...
            $calendar.= "<td class='calendar-today'  onclick='show_alert($date);' data-date=$date>";
          } else {
            $calendar.= "<td class='calendar-day'  onclick='show_alert($date);' data-date=$date>";
          }
...
function list_of_note_cust() { //zoznam vsetkych zakaznikov, ktory maju nejaku note
...
function list_of_task_cust() { //zoznam vsetkych zakaznikov, ktory maju nejaku note
                          global $con;
                          $task_cust="<ul>";
...
function list_of_task_cats() { //zoznam vsetkych zakaznikov, ktory maju nejaku note
                          global $con;
                          $task_cats="<ul>";
 ...
 function list_of_events_by_cust()
 {
  ...
  function show_event_detail($event_id) {
                      global $con;
                       ...
                       //echo "$sql";
                       $result=mysqli_query($con, $sql) or die('MySQL ERROR: '.mysql_error($con));
                       $row = mysqli_fetch_array($result); {
...
function GetNoteIdsByTags($cat_id) { //zoznam vsetkych note id pre danu kateogriu
  global $con;
...
function GetNoteTagsIds($note_id){//zoznam vsetkych ideciek kategorii pre danu note id
  global $con;
